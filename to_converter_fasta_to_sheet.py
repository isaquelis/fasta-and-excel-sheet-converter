...
seq_sheet = seq_wb.active # Activing workbook as a sheet object.
...
for i in range(2, len(sequence)+2):
    sequence_slices = re.split('[|]*[|]\s', sequence[i-2].description) # Splliting FASTA description to make different columns.

    # Contitional that verify if the description slice is empty.
